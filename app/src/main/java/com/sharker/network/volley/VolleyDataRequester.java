...
public class VolleyDataRequester {
    ...
    public VolleyDataRequester(Context ctx, Type type) {
        if (type == Type.HTTP) {
            mRequestQueue = VolleyHelper.getVolley().getRequestQueueWithHttp();
        }

        if (type == Type.HTTPS_DEFALT) {
            mRequestQueue = VolleyHelper.getVolley().getRequestQueueWithDefaultSsl();
        }

        if (type == Type.HTTPS_SELF_CERTIFIED) {
            mRequestQueue = VolleyHelper.getVolley().getRequestQueueWithSelfCertifiedSsl();
        }
